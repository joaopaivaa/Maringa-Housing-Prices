...
for (j in 1:num_pages){
  ...
  for (i in 1:length(urls)){
    ...
    price <- main_page %>% html_nodes("h3") %>% html_text() %>% .[i+1] %>% str_extract("[0-9,.]+") %>% gsub("\\.", "", .) %>% gsub(",", ".", .) %>% as.numeric(.)
    ...
    area <- property_page %>% html_nodes('div.col-4.mb-4') %>% html_nodes('span') %>% html_text(trim=TRUE) %>% .[3] %>% str_extract("[0-9,.]+") %>% as.numeric(.)
...
for (j in 1:num_pages){
  ...
  for (i in 1:length(urls)){
    ...
    price <- main_page %>% html_elements("span.list__price") %>% .[seq(1, length(.), by = 2)] %>% html_text() %>% .[i] %>% str_extract("[0-9,.]+") %>% gsub("\\.", "", .) %>% gsub(",", ".", .) %>% as.numeric(.)
...
for (j in 1:num_pages){
  ...
  for (i in 1:length(urls)){
    ...
    price <- main_page %>% html_elements("div.lista-imoveis-detalhes") %>% html_nodes("span.valor") %>% .[i] %>% html_text() %>% str_extract("[0-9,.]+") %>% gsub("\\.", "", .) %>% gsub(",", ".", .) %>% as.numeric(.)
...
#dbSendQuery(con, "CREATE DATABASE maringa_properties")

# Past properties ##############################################################

#dbExecute(con, "DROP TABLE displayed_properties")

past_properties <- dbReadTable(con, "displayed_properties")

# Criar tabela no data warehouse (se necessário)
dbExecute(con, "
  CREATE TABLE IF NOT EXISTS displayed_properties (
...
dbWriteTable(con, "displayed_properties", displayed_properties, overwrite = TRUE, row.names = FALSE)
...
#dbExecute(con, "DROP TABLE sold_properties")
...
dbWriteTable(con, "sold_properties", sold_properties, append = TRUE, row.names = FALSE)
...
#dbExecute(con, "DROP TABLE properties_to_sold")

# Criar tabela no data warehouse (se necessário)
dbExecute(con, "
  CREATE TABLE IF NOT EXISTS properties_to_sold (
    added_date DATE,
    property_url VARCHAR(255),
    district VARCHAR(255),
    price DECIMAL(15, 2),
...
dbWriteTable(con, "properties_to_sold", new_properties, append = TRUE, row.names = FALSE)
